...
To explore the dataset, `str()` function was used in code below:

```{r Exploring the Dataset}
str(data)
```
After looking at the dataset structure and comparing it with the dataset description, i believe the variable classification should be as follow:

**1. Numeric**: *enrltot*, *teachers*, *calwpct*, *mealpct*, *computer*, *testscr*, *compstu*, *expnstu*, *str*, *avginc*, *elpct*, *readscr*, and *matchscr*.

**2. Categorical**: *distcod*, *county*, *district*, and *grspan*.
...
The dataset has been filtered to only include data from those counties as *data_filtered*
...
The scatterplot between *readscr* and *mealpct* above indicates that the two variables display negative linear relationship meaning that as the percentage of students on free and reduced lunch increases, the average reading score tends to decrease. The trend line overlay-ed on top of the points further emphasize the relationship between the two variables and represent the overall trend in the data.

Frankly speaking, intuitively i would have expected the opposite of this result since i once read a health article discussing about student who are having lunch may actually increase their overall score. I even try to calculate the correlation value and found the same result with the above plot.

But after i did some googling, it gradually make sense to me in a way that *mealpct* might be used as proxy for the poverty. The higher the number of student eligible for free/reduced price for lunch might translate to the higher aggregate of family with poverty in that school/district. I need to do more research on this but my hypothesis is that the lower income family students might not have the same privileges as their higher income family peers thus resulting in lower avg reading score but again correlation does not imply causation so it is only my hunch and i need to do more research.
